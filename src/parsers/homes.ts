...
export function parseHomes(document: Document, url: string): ParseResult {
  if (!url.includes("homes.co.jp")) return parseGeneric(document, url);
  ...
  const priceEl = document.querySelector(".priceLabel, [class*='price']");
  ...
  if (Object.keys(candidates).length < 5) warnings.push("LIFULLHOMESパーサーで取得項目が少ない");

  return { source: "homes", candidates, confidence, warnings };
}
